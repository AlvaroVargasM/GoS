...
public class LinkedList<GenericType>{
    ...
    public void selectionSort(){
        if(firstNode != null && firstNode.getData().getClass() == Integer.class){
    ...
    private void selectionSort(boolean ascending){
        for(LinkedListNode node1 = getFirstNode(); node1.getNextNode() != null;
            ...
            Integer intInNode1 = (Integer) node1.getData();
            for(LinkedListNode node2 = node1.getNextNode(); node2 != null;
                node2 = node2.getNextNode()){
                Integer intInNode2 = (Integer) node2.getData();
                if(intInNode1 > intInNode2){
    ...
    private void insertionSort(boolean ascending){
        ...
        for(int i = 1; i < lenght; i++){
            Integer integer1 = (Integer) getNodeInPosition(i).getData(); 
            int j = i-1;
            Integer integer2 = (Integer) getNodeInPosition(j).getData();
            if(ascending){
                while (j>=0 && integer2 > integer1){ 
                    getNodeInPosition(j+1).setData(getNodeInPosition(j).getData());
                    j--;
                    integer2 = (Integer) getNodeInPosition(j).getData();
                }
            }else{
                while (j>=0 && integer2 < integer1){ 
                    getNodeInPosition(j+1).setData(getNodeInPosition(j).getData());
            ...
            }getNodeInPosition(j+1).setData(integer1);
        }
    }
